                         ...
                         const std::map<std::string, std::variant<int, float, std::string>> &dictionary)
{
    ...
    if (tStdHepPdg[j] == 22 && fKE_gamma >= 2 * electron_ke)
    {
        ...
        pdgs.push_back(22);
        masses.push_back(0.0); // ← FIXED: Explicitly set photon mass to 0.0
    ...
    else if ((tStdHepPdg[j] == 2212 && fKE >= prot_ke) ||
...
void VectorLept_w0NC(const std::string &input_file)
{
    ...
    std::string Output_Root_file = add_file + "VectorLeptw0NC_eventnum_" + num_events_str + "_" + proton_num_str + "p" + pion_num_str + "pi.root";

    TFile *treefile = new TFile(Output_Root_file.c_str(), "RECREATE");

    std::string output_name = std::get<std::string>(dictionary["OUTPUT_NAME"]);
    std::string outfile_name = output_name + "VectorLeptw0NC_eventnum_" + num_events_str + "_" + proton_num_str + "p" + pion_num_str + "pi.csv";
    ...
    std::string directory = output_directory + "VectorLeptw0NC_eventnum_" + num_events_str + "_" + proton_num_str + "p" + pion_num_str + "pi";
