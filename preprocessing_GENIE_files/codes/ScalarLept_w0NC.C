                         ...
                         const std::map<std::string, std::variant<int, float, std::string>> &dictionary)
{
    ...
    // Handle photons (massless particles) separately
    if (tStdHepPdg[j] == 22 && fKE_gamma >= 2 * electron_ke)
    {
        pdgs.push_back(22);
        masses.push_back(0.0); // ← KEY FIX: Explicitly set photon mass to 0.0
...
void ScalarLept_w0NC(const std::string &input_file)
{
    ...
    std::string Output_Root_file = add_file + "ScalarLeptw0NC_eventnum_" + num_events_str + "_" + proton_num_str + "p" + pion_num_str + "pi.root";

    TFile *treefile = new TFile(Output_Root_file.c_str(), "RECREATE");

    std::string output_name = std::get<std::string>(dictionary["OUTPUT_NAME"]);
    std::string outfile_name = output_name + "ScalarLeptw0NC_eventnum_" + num_events_str + "_" + proton_num_str + "p" + pion_num_str + "pi.csv";
    ...
    std::string directory = output_directory + "ScalarLeptw0NC_eventnum_" + num_events_str + "_" + proton_num_str + "p" + pion_num_str + "pi";
